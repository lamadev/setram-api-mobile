	...
	if($IdAgence=='inconnu'){
	 ...
	 if($Formulaire_Valide == true)
	 {
		//$IdAgence=$_SESSION['IdAgence'];
			$account=new Compte();
			$IdCompteB=$account->getIdCompteBeneficiary($IdCompteB);
			//echo "IdCompteB:".$IdCompteB."\n";
			//echo "IdCompteE:".$IdCompteE."\n";
		$Resultat=Transferer($MontantTransfert, $IdCompteB, $IdCompteE, $CodeMonnaietrans, $Moyen, $idagent, $IdAgence);
		if($Resultat=='1')
	 	{
			echo trim($account->fx_info_account($IdCompteE));
			
		}else{
			echo "Response:".$Resultat;
		}
	 }else{
		echo 9;	
     }
